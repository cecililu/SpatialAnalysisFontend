...
const App = () => {
  ...
  const fetchBufferPolygon = async (latlng) => {
    const data = await fetch(
      `http://127.0.0.1:8000/building/?lat=27.661485423990104&lng=85.32102656735674&buffer_distance=100`
    );
  ...
  function createPopupContent(feature) {
    return "<h3>" +'OSM ID FOR THE BUIDING' + "</h3>" +
           "<p>" + feature?.properties?.osm_id + "</p>"+
          "<p> Amenities" + feature?.properties?.amenities + "</p>";
  }

  function onEachFeature(feature, layer) {
    var color=feature.properties.amenities=='place_of_worship'?'blue':'red' 
    var fillcolor=feature.properties.amenities=='place_of_worship'?'blue':'' 
